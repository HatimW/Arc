...
function parseLectureTags(raw) {
  ...
  if (Array.isArray(parsed)) {
    return normalizeLectureRefs(parsed);
  }
  ...
  const refs = entries.map(entry => {
    const [blockId, id, name, week] = entry.split(/[:|]/).map(part => part.trim());
  ...
  return normalizeLectureRefs(refs);
}
